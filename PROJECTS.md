...
신호림의 프로젝트를 **주제 · 목적 · 동기 · 세부사항** 순서로 정리한 문서입니다.
구성은 [프로필 README](./README.md)와 같습니다.

| 구분 | # | 프로젝트 | 분야 | 주요 기술 | 형태 |
| --- | --- | --- | --- | --- | --- |
| 🚀 Featured | 1 | [아이앞으로](#1-아이앞으로-foryouhana) | 금융 웹 서비스 | Next.js · React · TypeScript · Prisma · MySQL | 팀 (8인) |
| | 2 | [하나케어](#2-하나케어-hanarocare) | 금융 웹 서비스 | Spring · MySQL · React · TypeScript | 팀 (6인) |
| 📂 Projects | 3 | [battery-soh-pipeline](#3-battery-soh-pipeline) | 데이터 분석 | Python · scikit-learn · FastAPI · Docker · GitHub Actions | 개인 |
| | 4 | [QuantInvesting](#4-quantinvesting) | 금융 | Python · vectorbt · Streamlit · Plotly | 개인 |
| | 5 | [mstar-atr](#5-mstar-atr) | 데이터 분석 | Python · scikit-image · scikit-learn · FastAPI | 개인 |
| 📚 개인 공부 | 6 | [hanaro_first_hw](#6-hanaro_first_hw) | DB 모델링 | Next.js · TypeScript · Prisma · MySQL | 교육 과제 |
| | 7 | [hana8](#7-hana8) | 학습 기록 | HTML · CSS · JavaScript · TypeScript | 교육 과정 |
| 🧩 Algorithm | 8 | [PracTest](#8-practest) | 코딩 테스트 | C++ · Python · SQL | 개인 |
...
- 팀 구성: 8명 (Team F1)
- **내 역할**
  - **타임라인·자녀 프로필 화면 프론트엔드 개발**
    - 자녀별 타임라인: 서버 컴포넌트에서 자녀 정보, 자녀 목록, 타임라인, 계좌를 병렬 조회해 이벤트 카드로 렌더링
    - 부모 메모 작성: Server Action으로 메모를 저장하고 `revalidatePath`로 화면에 즉시 반영
    - 자녀 프로필 변경 화면과 성인 자녀 안내 팝업, 타임라인 컴포넌트 Storybook 스토리
  - **데이터베이스 설계**: 타임라인 테이블, 연금저축펀드 계좌 등 Prisma 스키마 설계·변경과 시드 데이터 구성
...
**동기**
투자 전략을 감이 아닌 데이터로 검증하는 퀀트 투자의 과정을 데이터 수집부터 시뮬레이션, 시각화까지 직접 구현해 보고 싶었습니다.

**세부사항**
- **데이터**: yfinance로 과거 주가를 수집합니다. 미국 주식, 암호화폐, 국내 주식(`.KS` / `.KQ`)을 지원합니다.
- **전략 엔진** (`src/backtest.py`)
  - vectorbt로 단기·장기 이동평균선을 계산합니다.
  - 교차 시그널이 나온 다음 거래일 종가에 매수·매도하는 포트폴리오를 시뮬레이션합니다(look-ahead bias 방지). 수수료도 반영합니다.
  - 실제 주문 기능은 없는 백테스트(시뮬레이션) 도구입니다.
